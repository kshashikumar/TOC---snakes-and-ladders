...
public class Main extends JFrame {
	...
	private int player = 1; // present player index number.
	private int[] flag = new int[6]; // every player will start the game,if flag == 1
	private int[] playerposition = new int[4]; // 4 players
	private int point;
	...
	private int discImagePosition = 0, playerItem = 0;
	...
	public void Dice() {
		...
		if (point == 1)
			flag[player - 1] = 1; // If Player's got a 1, he can start the game
		if (flag[player - 1] == 1)
			movedisc();
	}

	public static void main(String[] args) { // No need to enter here
	...
	public void movedisc() {
		// Increase count if the playtime is raised and if not, the count is 0
		if (point == 6)
			playerItem++;
		else
			playerItem = 0;
		...
		if (playerItem > 0)
			discImagePosition = playerposition[player - 1];
		if (playerposition[player - 1] + point > 100) { // If the player's position exceeds 100, it will remain in the same position.
			playerposition[player - 1] = playerposition[player - 1];
		}

		// If player continously gets 6 three times, then he will be held in the same previous position.
		else if (playerItem == 3) {
			playerposition[player - 1] = discImagePosition;
		} else {
			RemoveImage(playerposition[player - 1]); // Player will now moved to the new position.
			playerposition[player - 1] += point; // Updating player position

			SetImage(playerposition[player - 1], player); // updating the image in new position.

			SkipTest(playerposition[player - 1]); // alerting for snake bites or stair climbs

			if (playerposition[player - 1] != Skipper(playerposition[player - 1])) { // What should we do if the snake is bitten or stair found?
	...
	public void PositionCheck(int x) { // Hold two people on same block.
	...
	public Main() {
		setTitle("TOC - Snakes and Ladders game"); 
		FrameinMiddle();
		initialize();
	}


	public void SkipTest(int score) { // This will show whether a snake ate or player took a stair
	...
	// Labels of the game board and labels of the rise should all be initialized
	...
	public void initialize() {
		...
		contentPane.setBackground(new Color(48, 41, 48));
		...
		btnDice.setForeground(new Color(0, 0, 0));
		...
		btnDice.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Dice();
				if (playerItem == 0) {
					player++; // updating player count, for next players chance
					if (player == 5)
						player = 1;
				}
		...
		// Below code is to check on enter key inputs without pointer
	...
	// If the snake is bitten, the player will go to another place
